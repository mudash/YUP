...
#$pkg_shasum="2167d9afeee9ec47e62f460ae20e20f19c9b379e365027704064948681d6114a"
...
$pkg_upstream_url="https://github.com/mudash/YUP"
...
function Invoke-Unpack {
  write-output "Attempting to clone from Git"
  write-output "Hab Cache Source Path: $HAB_CACHE_SRC_PATH"
  cd $HAB_CACHE_SRC_PATH
  git clone https://github.com/mudash/YUP.git yupsrc
...
function Invoke-Build {
  hab pkg binlink core/go
  cd $HAB_CACHE_SRC_PATH\yupsrc
  go get -d ./...
  go build -o YUP.exe
  go version
  #Invoke-DefaultBuild
...
function Invoke-Install {
  Copy-Item -Path "$HAB_CACHE_SRC_PATH\yupsrc\YUP.exe" -Destination "$pkg_prefix\bin" -recurse 
...
function Invoke-End {
  Remove-Item "$HAB_CACHE_SRC_PATH\yupsrc" -Recurse -Force
  #Invoke-DefaultEnd
}
